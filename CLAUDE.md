
# 代码风格
所有日志必须使用英文。
日志使用 from scripts.utils import stock_logger， stock_logger.debug 或 .error 进行输出。不要使用原生的 print。
所有 import 必须在文件头。除非形成循环import时才能在函数内进行 import。
完成代码修改后，不要尝试执行代码，由我来手动执行测试结果。
你只能对修改这几个文件夹：[项目workspace]/scripts，[项目workspace]/data。对于其他文件夹都只能进行读取。
在修复问题后，应当尽量将修复问题的地方写上注释，记录这里是为了解决什么问题或需求。

# 计划模式
如果我的需求是列出 plan_xxx.md 计划。那么列完计划之后不要立刻执行计划，不要修改代码。

# 解决bug时的手段
可以加入更多调试日志来定位 bug。

# 项目可用的 python packages
见 @requirements.txt。

# 项目结构
本项目为纯python，位于 /Users/mac/virtualenvs/venv_baostock/ 中。
venv_baostock 是一个 python 虚拟环境文件夹。
非第三方package的，自己编写的脚本文件都放在 ./scripts。
scripts/database_ops：数据库访问操作。
scripts/utils：通用基础操作函数。
scripts/mail：邮件相关操作。
scripts/draw: 画股票指标图。
scripts/news_monitor: 实时新闻监控。
scripts/akshare: submodule，从 git fork 的 akshare 股票功能库。
scripts/data_analyzed：存放临时分析的数据结果，中间结果等。
scripts/sginal_catcher: 股票实时指标监控


